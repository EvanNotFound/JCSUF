...
function compileTemplate(templateId, data, targetId) {
  ...
  xhttp2.onreadystatechange = function() {
	  if (this.readyState == 4 && this.status == 200) {
		  ...
		  var modifyTemplate = document.getElementById("notice-modify-template").innerHTML;
		  var compiledModifyTemplate = modifyTemplate.replace(/{{(\w+)}}/g, function(match, p1) {
			  return templateData[p1];
		  });
		  document.getElementById("notice-modify-time").innerHTML = compiledModifyTemplate;
	  }
  };
	...
	xhttptrans.onreadystatechange = function() {
	  if (this.readyState == 4 && this.status == 200) {
		...
		if (currentid == art.author) {
		  ...
		  compileTemplate("article-info-template", templateData, "article-info-" + art.id);
		} else {
		  var templateData = {
			author: responseData.name,
			isCurrentUser: false,
