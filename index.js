...
xhttp4.onreadystatechange = function() {
	if (this.readyState == 4 && this.status == 200) { nettaskfinish()
		...
		for(let i=0;i<rarts.length;i++){
			...
			document.getElementById("article-info-"+rarts[i].id).innerHTML = '<a href="user.html?uid='+rarts[i].author+'" class="post_author">'+rarts[i].author_name+'</a> 发布于 '+formatDateTime(rarts[i].ctime)+'<br><span style="margin-left:18px">'+rarts[i].ccount+' 评论</span><br><span style="margin-left:18px">'+rarts[i].view+' 阅读</span>'
			transname(rarts[i])
		}
	}
};
